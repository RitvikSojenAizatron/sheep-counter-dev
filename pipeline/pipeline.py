...
from pipeline.unified_streamer import UnifiedStreamer

logger = logging.getLogger(__name__)


class AnnotationSink:
    """Callable sink for InferencePipeline: annotates frames and pushes to UnifiedStreamer."""
    ...
    def _draw_counts(frame: np.ndarray, linezones: dict) -> None:
        ...
        cv2.rectangle(frame, (x - pad, pad), (frame.shape[1] - pad, pad + block_h), (0, 0, 0), -1)
...
class Pipeline:
    ...
    the annotated stream via RTMP to MediaMTX."""
    ...
    def __init__(
        ...
        self._streamer = UnifiedStreamer(rtmp_url)

        self._annotation_sink = AnnotationSink(
    ...
    def start(self) -> None:
        ...
        source = active[0]
        rtsp_url = source.ip_address
        w = source.resolution_width or 1920
        h = source.resolution_height or 1080

        self._streamer.start(fps=25, width=w, height=h)
        ...
        self._inference_pipeline = InferencePipeline.init_with_custom_logic(
            ...
            on_prediction=self._annotation_sink,
    ...
    def _maybe_log_stats(self) -> None:
        ...
        logger.info(
            "stats | frames=%d actual_fps=%.1f stream_fps=%.1f latency=%.0f ms counts=%s",
            self._frame_count,
            actual_fps,
            self._current_fps,
            self._frame_latency_ms,
    ...
    def request_recording_start(self) -> None:
        ...
        self._streamer.start_recording(output_path)
