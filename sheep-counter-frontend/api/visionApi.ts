...
export interface FetchServiceLogsParams {
  service?: string;
  limit?: number;
  since?: string;
}
...
export const sheepCounterApi = {
  ...
  startRecording: () =>
    apiClient.post('/api/live/recording/start').then((res) => res.data),
  stopRecording: () =>
    apiClient.post('/api/live/recording/stop').then((res) => res.data),
  fetchHostname: () =>
    apiClient
...
export const visionApi = sheepCounterApi;
